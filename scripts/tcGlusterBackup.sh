...
export TC_GLUSTER_BRICK=TrustChain_Backup
export TC_BACKUP_PATH=/srv/TrustChain_Backup

export TC_GLUSTER_MANAGERS=("TC_SWARM_WORKER1" "TC_SWARM_WORKER2")

declare -A TC_MOUNT1=( [node]=${TC_SWARM_MANAGER1[node]} [ip]=${TC_SWARM_MANAGER1[ip]} [mnt]="$TC_BACKUP_PATH" )
declare -A TC_MOUNT2=( [node]=${TC_SWARM_MANAGER2[node]} [ip]=${TC_SWARM_MANAGER2[ip]} [mnt]="$TC_BACKUP_PATH" )
declare -A TC_MOUNT3=( [node]=${TC_SWARM_MANAGER3[node]} [ip]=${TC_SWARM_MANAGER3[ip]} [mnt]="$TC_BACKUP_PATH" )
declare -A TC_MOUNT4=( [node]=${TC_SWARM_WORKER1[node]} [ip]=${TC_SWARM_WORKER1[ip]} [mnt]="$TC_BACKUP_PATH" )
declare -A TC_MOUNT5=( [node]=${TC_SWARM_WORKER2[node]} [ip]=${TC_SWARM_WORKER2[ip]} [mnt]="$TC_BACKUP_PATH" )
export TC_GLUSTER_MOUNTS=()
TC_GLUSTER_MOUNTS+=("TC_MOUNT1")
TC_GLUSTER_MOUNTS+=("TC_MOUNT2")
TC_GLUSTER_MOUNTS+=("TC_MOUNT3")
TC_GLUSTER_MOUNTS+=("TC_MOUNT4")
TC_GLUSTER_MOUNTS+=("TC_MOUNT5")
...
function _glusterLay() {
	...
	local cmd="sudo gluster volume create $TC_GLUSTER_BRICK $servers"
...
function _glusterServerFstab() {
	...
	_inner() {
		...
		commonPrintf "${peer[gdev]} ${peer[gmnt]} on ${peer[node]}"
	...
	_inner() {
		declare -n peer=$1
		declare -n server=${TC_GLUSTER_MANAGERS[0]}
		declare -n backup=${TC_GLUSTER_MANAGERS[1]}
		if [[ "${peer[node]}" == "${backup[node]}" ]]; then
			declare -n server=${TC_GLUSTER_MANAGERS[1]}
			declare -n backup=${TC_GLUSTER_MANAGERS[0]}
		fi 
		...
		commonPrintf "appending new entries"
		commonPrintf "backupvolfile-server=${backupvol}"
		commonPrintf "$TC_BACKUP_PATH on ${peer[node]}"
		entry="${server[node]}:/${TC_GLUSTER_BRICK} $TC_BACKUP_PATH glusterfs defaults,_netdev,backupvolfile-server=${backup[node]} 0 0\n"
		_out=$( ssh ${peer[node]} "sudo bash -c 'echo -e \"$entry\" >> /etc/fstab'" 2>&1 )
		commonVerify $? "failed to update fstab: $_out" "fstab update succeeded"

		commonPrintf "mkdir -p -v ${peer[node]}:${TC_BACKUP_PATH}"
		_out=$( ssh ${peer[node]} "sudo mkdir -p -v ${TC_BACKUP_PATH}" 2>&1 )
		commonVerify $? "failed: $_out" "mkdir -p -v ${TC_BACKUP_PATH} on ${peer[node]} succeeded"

		commonPrintf "mount -a at ${peer[node]}"
		_out=$( ssh ${peer[node]} "sudo mount -a" 2>&1 )
		commonVerify $? "failed mount -a: $_out" "mount -a succeeded"
	}
	commonIterate _inner "confirm|update fstab and mount -a on server |array|node|?" "${TC_GLUSTER_MOUNTS[@]}"

	# endregion: clients

	commonPrintf "chgrp and chmod g+rwx"
	local grp=$( id -g )
	_out=$( sudo chgrp $grp "$TC_BACKUP_PATH" )
	commonVerify $? $_out
	_out=$( sudo chmod g+rwx "$TC_BACKUP_PATH" )
	commonVerify $? $_out
