...
[[ "$TC_EXEC_DRY" == false ]] && commonYN "bootstrap ${TC_ORDERER1_STACK}, enroll ca admin, and register identities?" _Orderer1CA
...
_Org2CA() {
	...
	commonVerify $? "failed to register ${TC_ORDERER1_ADMIN}: $out" "$out"
	# local out=$( fabric-ca-client register --id.name $TC_ORG2_USER --id.secret $TC_ORG2_USERPW --id.type user -u https://0.0.0.0:${TC_ORG2_C1_PORT} 2>&1 )
	# commonVerify $? "failed to register ${TC_ORDERER1_ADMIN}: $out" "$out"
	# local out=$( fabric-ca-client register --id.name $TC_ORG2_CLIENT --id.secret $TC_ORG2_CLIENTPW --id.type client -u https://0.0.0.0:${TC_ORG2_C1_PORT} 2>&1 )
	# commonVerify $? "failed to register ${TC_ORDERER1_ADMIN}: $out" "$out"

	# endregion: register org2 users
	# region: register org2's gw and peers

	commonPrintf "registering ${TC_ORG2_STACK}'s gw and peers with $TC_ORG2_C1_FQDN"
	# local out=$( fabric-ca-client register --id.name $TC_ORG2_G1_CA_NAME --id.secret $TC_ORG2_G1_CA_PW --id.type orderer -u https://0.0.0.0:${TC_ORG2_C1_PORT} 2>&1 )
...
# [[ "$TC_EXEC_DRY" == false ]] && commonYN "bootstrap ${TC_ORG2_STACK}, enroll ca admin, and register identities?" _Org2CA
